...
class RSDataset:
    def __init__(
        ...
        if mode == Mode.train:
            if use_chusai:
                ...
                self.list_path = f'{root}/train/train_segmentation_only.txt'
            ...
            pose_test = pd.read_csv(f'{root}/pos_test.csv')
        else:
            raise ValueError('Mode error')
        
        if mode != Mode.predict_cut:
            with open(self.list_path, mode='r') as file:
                img_list = [line.strip() for line in file]
        else:
            img_list = pose_test['slice_name'].to_list()
        ...
        if mode == Mode.train:
            ...
            self.img_list = [
                (f'{root}/valid/images/{filename}', f'{root}/valid/masks/{filename}', filename)
                for filename in img_list
            ]
        elif mode == Mode.predict:
            self.img_list = [
    ...
    def get_transform(self):
        if self.mode == Mode.train:
            ...
            return TransformPred_Cut(self.crop_size[0], self.mean, self.std)
        else:
            return None
