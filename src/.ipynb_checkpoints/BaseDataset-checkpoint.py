...
class BaseDataset:
    ...
    def gen_image(self, image):
        """Data preprocessing."""
        image = self.multi_scale_aug(image, label=None, rand_scale=1, rand_crop=False)
        resize_shape = image.shape[:2]
        # image = self.pad_image(image, resize_shape[0], resize_shape[1], (self.base_size, self.base_size),
        #                        (0.0, 0.0, 0.0))
        image = self.input_transform(image)  # HWC
        image = image.transpose((2, 0, 1))  # CHW
        # image CHW, label HW
